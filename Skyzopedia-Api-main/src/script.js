...
function setupUI() {
    ...
    if (titleApi) titleApi.textContent = settings.name || "Skyzopedia API";
    if (descApi) descApi.textContent = settings.description || "Interactive API documentation with real-time testing";
    if (footer) footer.textContent = `© ${new Date().getFullYear()} ${settings.creator || "Skyzopedia"} - ${settings.name || "Skyzopedia API"}`;
